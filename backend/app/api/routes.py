...
async def get_trading_days():
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving trading days: {str(e)}"
        )

@router.get("/data/{date}", response_model=APIResponse)
async def get_trading_day_data(date: str):
    """Get complete trading day data including trade markers"""
    try:
        ...
        # Get data
        data = data_service.get_trading_day_data(date)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving data for {date}: {str(e)}"
        )

@router.get("/data/{date}/summary", response_model=APIResponse)
async def get_day_summary(date: str):
    """Get summary data for a trading day (faster endpoint)"""
    try:
        # Validate date format
        from datetime import datetime
        try:
        ...
        # Get summary
        summary = data_service.get_day_summary_only(date)
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error retrieving summary for {date}: {str(e)}"
        )
